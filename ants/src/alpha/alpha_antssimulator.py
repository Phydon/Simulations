...
class Objects():
    def draw_obj(self):
        if self.alive == 1:
            surface.blit(
                self.image, (self.position.x, self.position.y))
...
class Entity(Objects):
    def __init__(self, pheromone_colour,
        ...
        self.image = pg.transform.scale(pg.image.load(
            os.path.join(imagefolder, image)).convert(), (self.width, self.height))
    ...
    def random_walk(self, position, settings):  # moving randomly
        ...
        if num == 0 and position.x - \
                self.velocity > settings['border_left'].width:
            position.x -= self.velocity

        # right
        elif num == 1 and position.x + self.velocity + position.width < settings['border_right'].left:
            position.x += self.velocity
        ...
        # down
        elif num == 3 and position.y + self.velocity + position.height < settings['border_down'].top:
            position.y += self.velocity
    ...
    def reproduce(self, target_lst):
        if self.energy >= 1000:
            ...
            mutant = Entity(
                ...
                'redpixel.jpg', settings)
            mutant.mutate(settings)
            print('Aggression:', mutant.attack, '\n',
                  'Velocity:', mutant.velocity, '\n',
                  'Energy:', mutant.energy, '\n',
                  'Size:', mutant.width, 'x',
                  mutant.height, '\n',
                  'Image:', mutant.image, '\n',
                  'Rect:', mutant.rect)
    ...
    def mutate(self, settings):
        ...
        self.width += settings['size_mutation']  # TODO: not working
        self.height += settings['size_mutation']  # TODO: not working
        return self.attack, self.velocity, self.energy, self.width, self.height
...
class Home(Objects):
    def __init__(self, imagefolder, image):
        self.image = pg.transform.scale(pg.image.load(
            os.path.join(imagefolder, image)).convert(), (settings['home_width'], settings['home_height']))
        self.rect = self.image.get_rect()
...
if __name__ == '__main__':
    ...
    pg.display.set_caption('Reproduction Simulation')
